...
public abstract class DataFolder {
     ...
     */
    public DataFolder(String folderName) {
    ...
    public void init() throws NoUserJsonFoundException {
        File parent = new File(LuaCore.getUserDataManager().getUserFolderPath(folderName));
    ...
    public <T> T loadData(@NotNull Class<T> clazz, @NotNull String fileName) {
        T data = null;
        try {
            File file = FileManager.file(LuaCore.getUserDataManager().getRelativeUserFolderPath(folderName), fileName);
